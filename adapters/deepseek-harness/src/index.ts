...
export async function answerReviewQuestions(
  ...
  try {
    ...
    return plan.review_questions.slice(0, MAX_REVIEW_QUESTIONS).map((question) => {
      ...
      if (!option) throw new Error(`Context Guardian received an unknown answer for ${question.id}`);
      return {
        question_id: question.id,
        topic_id: question.topic_id,
        action: option.id,
...
function normalizeInput(input: NativeSummarizeInput): ReturnType<typeof normalizeDeepSeekMessages> {
