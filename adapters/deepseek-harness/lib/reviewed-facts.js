...
const BLOCK = new RegExp(`${escapeRegExp(REVIEWED_FACTS_START)}([\\s\\S]*?)${escapeRegExp(REVIEWED_FACTS_END)}`, "g");
...
function addFact(facts, seen, text) {
...
function priority(origin) {
    return origin === "human_keep" ? 3 : origin === "auto_correction" ? 2 : 1;
}
function mergeFacts(base, incoming) {
    const merged = [...base];
    for (const candidate of incoming) {
        if (merged.some((fact) => fact.text.toLocaleLowerCase() === candidate.text.toLocaleLowerCase()))
            continue;
...
function render(language, facts) {
    if (facts.length === 0)
        return "";
...
export function appendReviewedFacts(preview, appendix) {
    const matches = [...preview.matchAll(BLOCK)];
    if (matches.length === 0 && appendix.facts.length === 0)
        return preview;
    const carried = [];
    const seen = new Set();
    for (const match of matches) {
        for (const line of (match[1] ?? "").split("\n")) {
            if (line.trim().startsWith("- "))
                addFact(carried, seen, line.trim().slice(2));
        }
    }
...
//# sourceMappingURL=reviewed-facts.js.map