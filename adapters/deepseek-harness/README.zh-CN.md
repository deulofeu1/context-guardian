...
复现已发布配置时请固定两边的版本；例如 `0.3.3` 配对安装如下：
...
必须逐字匹配对应原文，才能进入 Review 主题或 Reviewed Facts。模型生成的易读或翻译文案
只用于界面显示；界面会单独展示原文证据，并且只持久化经过校验的原文完整句子。宿主规划
元数据、system/plugin 消息、工具调用、路径、哈希、日志和已解决的机械错误不会进入人工
审查。这与 Pi 适配器使用同一套来源规则。
