...
export class GuardianBridge {
  ...
  private async request(
    ...
    try {
      ...
      writeFrame({ protocol_version: PROTOCOL_VERSION, type: "request", request_id: requestId, operation, ...body });
      ...
      for await (const line of readline) {
        ...
        if (frame.type === "provider_request") {
          ...
          try {
            const data = await generateStructuredWithHost(ctx, frame.prompt ?? "", frame.schema ?? {}, operationController.signal);
            writeFrame({
